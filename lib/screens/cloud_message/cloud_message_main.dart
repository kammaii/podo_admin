...
import 'package:podo_admin/screens/cloud_message/cloud_message_reply_main.dart';
...
class CloudMessageMain extends StatelessWidget {
  ...
  final CloudMessageStateManager _controller = Get.put(CloudMessageStateManager());
  late HtmlEditorController _htmlEditorController;
  late CloudMessage selectedMsg;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('클라우드 메시지'),
      ),
      body: GetBuilder<CloudMessageStateManager>(
        builder: (controller) {
          if (_controller.messages.isNotEmpty) {
            return DataTable2(
              ...
              rows: List<DataRow>.generate(controller.messages.length, (index) {
                CloudMessage message = controller.messages[index];
                String status = '';
                      ...
                      getDialog(
                        ...
                        functionYes: () {
                          ...
                          if (value) {
                            ...
                            for (CloudMessage message in _controller.messages) {
                              if (message.isActive) {
